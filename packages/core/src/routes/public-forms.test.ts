...
// Mock createContentFromSubmission to capture what data it receives
vi.mock('../services/form-collection-sync', () => ({
  createContentFromSubmission: vi.fn().mockImplementation(
    (_db: any, data: any) => {
      capturedContentData = data
...
describe('POST /api/forms/:identifier/submit — XSS sanitization', () => {
  ...
  it('should pass sanitized data to createContentFromSubmission', async () => {
    const db = createMockDb()
    const app = createTestApp(db)
    ...
    // Content dual-write should also receive sanitized data
    expect(capturedContentData).toBeDefined()
    expect(capturedContentData.bio).toBe('&lt;script&gt;steal(document.cookie)&lt;/script&gt;')
    expect(capturedContentData.email).toBe('test@example.com')
  })

...
function bootstrapMiddleware(config = {}) {
  return async (c, next) => {
    ...
    try {
      ...
      const migrationService = new chunkJYPTQSJQ_cjs.MigrationService(c.env.DB);
      await migrationService.runPendingMigrations();
      console.log("[Bootstrap] Syncing collection configurations...");
      try {
        await chunkUS6ANQDX_cjs.syncCollections(c.env.DB);
      } catch (error) {
      ...
      if (!config.plugins?.disableAll) {
        ...
        const bootstrapService = new chunkUS6ANQDX_cjs.PluginBootstrapService(c.env.DB);
...
var requireAuth = () => {
  return async (c, next) => {
    try {
      ...
      if (!token) {
        try {
          const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
          await logger.warn("auth", "No auth token provided", { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", url: c.req.url });
        } catch {
        }
      ...
      if (!payload) {
        try {
          const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
          await logger.logAuth("token-verification", void 0, false, { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", userAgent: c.req.header("user-agent") || "", url: c.req.url, method: c.req.method });
        } catch {
        }
        const acceptHeader = c.req.header("Accept") || "";
        if (acceptHeader.includes("text/html")) {
      ...
      try {
        const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
        await logger.error("auth", "Auth middleware error", error, { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", userAgent: c.req.header("user-agent") || "" });
      } catch {
      }
      const acceptHeader = c.req.header("Accept") || "";
      if (acceptHeader.includes("text/html")) {
...
var requireRole = (requiredRole) => {
  return async (c, next) => {
    ...
    if (!roles.includes(user.role)) {
      try {
        const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
        await logger.logSecurity("insufficient-role", "medium", { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", userAgent: c.req.header("user-agent") || "", url: c.req.url, method: c.req.method, userId: user.userId });
      } catch {
      }
      const acceptHeader = c.req.header("Accept") || "";
      if (acceptHeader.includes("text/html")) {
...
var optionalAuth = () => {
  return async (c, next) => {
    try {
      ...
      try {
        const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
        await logger.warn("auth", "Optional auth error", { error: error instanceof Error ? error.message : String(error) });
      } catch {
      }
      return await next();
    }
  ...
  return async (c, next) => {
    ...
    if (apiKey) {
      ...
      if (!apiKey.scopes.includes(scope)) {
        try {
          const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
          await logger.logSecurity("api-key-insufficient-scope", "medium", { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", userAgent: c.req.header("user-agent") || "", url: c.req.url, method: c.req.method, keyId: apiKey.id, requiredScope: scope, actualScopes: apiKey.scopes });
        } catch {
        }
    ...
    if (enforce) {
      ...
      if (headerPresent) {
        try {
          const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
          await logger.logSecurity("api-key-invalid", "medium", { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", userAgent: c.req.header("user-agent") || "", url: c.req.url, method: c.req.method });
        } catch {
        }
      } else {
        try {
          const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
          await logger.logSecurity("api-key-required", "low", { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", userAgent: c.req.header("user-agent") || "", url: c.req.url, method: c.req.method });
        } catch {
        }
      }
  ...
  return async (c, next) => {
    ...
    try {
      ...
      if (entry.count > max) {
        ...
        try {
          const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
          await logger.logSecurity("rate-limit-exceeded", "medium", { ipAddress: ip, userAgent: c.req.header("user-agent") || "", url: c.req.url, method: c.req.method, keyPrefix, count: entry.count, max });
        } catch {
        }
      ...
      try {
        const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
        await logger.error("system", "Rate limiter error", error);
      } catch {
      }
      return await next();
    }
...
function csrfProtection(options = {}) {
  return async (c, next) => {
    ...
    if (c.env?.ENVIRONMENT === "production" && !c.env?.JWT_SECRET) {
      ...
      try {
        const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
        await logger.logSecurity("csrf-fallback-secret", "critical", { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", userAgent: c.req.header("user-agent") || "", url: c.req.url, method });
      } catch {
      }
    }
    ...
    if (!cookieToken || !headerToken) {
      try {
        const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
        await logger.logSecurity("csrf-token-missing", "medium", { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", userAgent: c.req.header("user-agent") || "", url: c.req.url, method });
      } catch {
      }
    ...
    if (cookieToken !== headerToken) {
      try {
        const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
        await logger.logSecurity("csrf-token-mismatch", "high", { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", userAgent: c.req.header("user-agent") || "", url: c.req.url, method });
      } catch {
      }
    ...
    if (!isValid) {
      try {
        const logger = chunkUS6ANQDX_cjs.getLogger(c.env?.DB);
        await logger.logSecurity("csrf-token-invalid", "high", { ipAddress: c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown", userAgent: c.req.header("user-agent") || "", url: c.req.url, method });
      } catch {
      }
...
var SKIP_PATHS = ["/health", "/favicon.svg", "/favicon.ico", "/admin/api/metrics"];
var requestLoggingEnabled = false;
var securityResponseLoggingEnabled = false;
function setRequestLoggingEnabled(enabled) {
  requestLoggingEnabled = enabled;
}
function setSecurityLoggingEnabled(enabled) {
...
function loggingMiddleware() {
  return async (c, next) => {
    ...
    try {
      ...
      const user = c.get("user");
      const logFn = () => logger.logRequest(c.req.method, url, c.res.status, duration, {
        ipAddress: ip,
        userAgent: ua,
        userId: user?.userId,
...
function performanceLoggingMiddleware(thresholdMs = 1e3) {
  return async (c, next) => {
...
async function logActivityFromContext(c, action, resourceType, resourceId, details) {
  const user = c.get("user");
  const db = c.env?.DB;
  const ip = c.req.header("cf-connecting-ip") || c.req.header("x-forwarded-for") || "unknown";
  const ua = c.req.header("user-agent") || "";
...
exports.logActivityFromContext = logActivityFromContext;
...
exports.setRequestLoggingEnabled = setRequestLoggingEnabled;
exports.setSecurityLoggingEnabled = setSecurityLoggingEnabled;
exports.syncFormCollection = syncFormCollection;
exports.validateCsrfToken = validateCsrfToken;
exports.verifySecurityConfig = verifySecurityConfig;
//# sourceMappingURL=chunk-RTMJPGZV.cjs.map
//# sourceMappingURL=chunk-RTMJPGZV.cjs.map